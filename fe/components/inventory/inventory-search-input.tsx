...
import {
  iProductVariant,
  iProductVariantSearch,
} from "@/components/variant/interface";
import PaginationHandle from "../ui-custom/pagination-handle";
import {
  addInventory,
  removeInventory,
...
export default function InventorySearchInput() {
    ...
    for (const key in inventorys) {
      ...
      html.push(
        ...
        >
          ...
          <div
            data-selected={s}
            className="data-[selected]:block data-[selected=false]:hidden absolute top-0 right-0 pr-10 pt-10"
          >
            <Check />
          </div>
        </li>
  ...
  return (
    <div className=" shadow-2xl p-4  rounded-lg">
      <h1 className=" mb-4">Chọn sản phẩm bầy bán</h1>
      <Sheet>
        <SheetTrigger asChild>
          <Button onClick={()=> setProductVariant([])} type="button" variant="blue">
            Sản phẩm
          </Button>
        </SheetTrigger>
        <SheetContent>
          ...
          <div className="mt-1 px-2 space-y-2.5 overflow-y-auto">
            {productVariant.map((v) => {
              return (
                <button
                  type="button"
                  data-selected={inventorys[v.productVariantId] != undefined}
                  onClick={() => {
                    handleSelect(v);
                  }}
                  key={v.productVariantId}
                  className="border-2 hover:shadow-2xl cursor-pointer rounded-2xl shadow-2xl data-[selected=false]:border-white data-[selected=true]:border-green-700 flex gap-2 p-2 shadow-2xs"
                >
                  <img src={v.image} className="aspect-square h-20" alt="" />
                  <p className="text-sm">{v.productVariantName}</p>
                </button>
            ...
            <div
              data-hidden={productVariant.length == 0}

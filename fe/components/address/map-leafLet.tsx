...
export default function MapLeafLet(p: { onChange?(lat: number, lng: number): void, latlng?: iLatLng }) {
        ...
        const f = async () => {
            ...
            var MyCustomControl = L.Control.extend({
                onAdd: function (maps: L.Map) {
                    ...
                    div.innerHTML = '<p class="bg-white p-2 cursor-pointer rounded-full"><img src="/locate-fixed.svg"/></p>';
                    div.onclick = () => {
            ...
            const map = L.map('maps').setView([latlng?.lat || 10.906132709550574,
            latlng?.lng || 106.8849634786165], 13);
    ...
    return (
        <div className="py-3.75">
            <div id="maps" ref={refmap} className="h-80 w-120">

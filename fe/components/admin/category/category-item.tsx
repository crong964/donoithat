...
function CategoryItem(p: { data: iMainCateGory }) {
  const path = usePathname();
  const mainCategory = p.data;
  const categoryItem = p.data.categoryChidlren;
  const s =
    mainCategory.categoryId != undefined &&
    path.indexOf(mainCategory.categoryId) >= 0;
  return (
    <Link
      href={`/admin/category/${s ? "" : mainCategory.categoryId}`}
      key={mainCategory.id}
    ...
    >
      ...
      <div className="basis-2/12">
        {mainCategory.categoryImage ? (
          <img
            src={mainCategory.categoryImage}
            alt={mainCategory.nameCategory}
            srcSet=""
          />
        ) : (
          <></>
        ...
        <section className="flex items-center mt-4">
          ...
          <div className="flex-1 px-3">
            <div className="flex flex-wrap gap-1">
              {categoryItem.map((option, i) => {
                return (
                  <span key={i} className="px-2 rounded-sm shadow-2xl border">
                    {" "}
                    {option.nameCategory}{" "}
                  </span>
                );

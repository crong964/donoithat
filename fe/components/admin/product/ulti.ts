...
const createClassificationFormSaveToHandle = (data: IProductClassification[]) => {

    let productClassification = [...data.map((v) => {
        return { ...v, options: [...v.options, createOption()] }
    })]
    return productClassification
}
const createClassificationFormHandleToSave = (data: IProductClassification[]) => {
    let tmpClassifications = [...data
        .map(v => {
            return {
                ...v, edit: false, options: v.options.
                    filter((_, ii) => {

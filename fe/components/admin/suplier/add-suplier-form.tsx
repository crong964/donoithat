...
import { addSuplier } from "@/service/admin/suplier-service";
import { CornerDownLeft, Plus, Save } from "lucide-react";
import Form from "next/form";
import Link from "next/link";
import { useActionState, useEffect, useState } from "react";
import { toast } from "react-toastify";

export default function SuplierAdminAddForm() {
    const [message, formAction, pending] = useActionState(addSuplier, null)
    ...
    return (
        <div className="p-4 flex">
            ...
            <div className="p-4 overflow-x-auto w-full">
                <p className="p-4 border border-[#00000014] mb-3.75"> Thêm nhà cung cấp</p>
                <Form action={formAction} >
                    <label className="flex mb-3.75" htmlFor="">
                        <p className="basis-sm">Mã nhà cung cấp <span className="text-f">*</span></p>
                        <Input
                            onChange={onChange}
                            required
                            value={data.suplierId}
                            title="chưa nhập"
                            name="suplierId" placeholder="Nhập mã nhà cung cấp" />
                    </label>
                    <label className="flex mb-3.75" htmlFor="">
                        <p className="basis-sm">Tên nhà cung cấp <span className="text-f">*</span></p>
                        <Input
                            required
                            onChange={onChange}
                            name="suplierName"
                    ...
                    <label className="flex mb-3.75" htmlFor="">
                        <p className="basis-sm">Điện thoại  <span className="text-f">*</span></p>
                        <Input required
                            onChange={onChange}
                    ...
                    <div className="justify-center flex">
                        <div>
                            <SubmitButton loading={<Button type="button" className="bg-loadingbg">
                                <Save /> Lưu
                            </Button>}>

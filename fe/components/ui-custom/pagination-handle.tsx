...
export default function PaginationHandle(p: {
  page: number;
  total: number;
  url: string;
  ...
  for (let i = p.page - 1; i > 0 && i >= p.page - margin; i--) {
    pre.push(
      <PaginationItem onClick={p.onClick} page={i} url={p.url} key={i} />
    );
  }
  ...
  const cur: JSX.Element = (
    <button className="bg-f rounded-sm text-white  text-sm px-3 py-1.5">
      {p.page}
    </button>
  );

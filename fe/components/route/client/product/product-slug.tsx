                      ...
                      return (
                        <li key={v} className="mb-2 w-17.25">
                          <span className="">
                            <img
                              className={`${
                                i == curIndexImage ? "border-f " : ""
                              } max-w-full h-auto  border`}
                              src={v}
                  ...
                  <MainCarousel>
                    {productImages.map((v) => {
                      return (
                        <li
                          key={v}
                          className="basis-22.5 shrink-0 grow-0 max-lg:pr-2"
                        >
                          <span className="w-full">
                            <img
                              className="max-w-full h-auto border-f border"
                              src={v}
                              alt={product.nameProduct}
                              srcSet=""
                            />
          ...
          <div className="lg:w-[55%]  lg:pl-3.75">
            <div className=" bg-white px-3.75">
              <div className="pt-3.75 mb-3.75">
                <h1 className="text-[24px] font-bold text-f mb-1.25 leading-7.25">
                  {product.nameProduct}
                </h1>
                <span className="text-[14px] flex flex-wrap gap-4.75 leading-5 ">
                  <span>
                    Mã sản phẩm:
                    <b className="text-f">
                      {productVariants[curIndexProductVariant].productVariantId}
                    </b>
                  </span>
                  <span>
                    Tình trạng:
                    <b className="text-f">
                      {curquality == 0
                        ? "hết hàng"
                        : ` ${curquality} sản phẩm có sẵn`}
                    </b>
                  </span>
                  <span>
                    Thương hiệu: <b className="text-f">{product.suplier}</b>
                  </span>
                </span>
              </div>
                ...
                <div className="flex flex-wrap max-lg:hidden -mx-1.75 -mb-2.5">
                  {Array.from({ length: 3 }).map((_, i) => {
                    return (
                      <div
                        key={i}
                        className="basis-1/2 grow-0 px-1.75 mb-3.75 "
                      >
                        <Coupon />
                      </div>
                    );
                  ...
                  <MainCarousel>
                    {Array.from({ length: 3 }).map((_, i) => {
                      return (
                        <div
                          key={i}
                          className="basis-6/7 shrink-0 grow-0 px-1.75 mb-3.75 "
                        >
                          <Coupon />
                        </div>
                      );

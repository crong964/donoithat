  ...
  return (
    <>
      ...
      >
          ...
          {category.map((v, i) => {
            return (
              ...
              >
                {v.name}
                <svg
                  className="absolute right-3.75 top-1/2 -translate-y-1/2 w-2 h-3"
                  viewBox="0 0 8 12"
                  role="presentation"
                >
                  <path
        ...
        >
          <button
            className="p-3.75 flex items-center w-full"
            onClick={() => {
              onChildrenDrawerClose();
            }}
          >
            <svg
              className=" rotate-180 w-2 h-3"
              viewBox="0 0 8 12"
              role="presentation"
            >
              <path
          ...
          <ul>
            {category[index]?.con?.map((v, i) => {
              return (
                <li
                  key={v.id}
                  className="p-3.75 uppercase text-[14px] border-b-boder border-b-2 "
                  onClick={() => {
                    showChildrenDrawer(i);
                  }}
                >

"use client";

import Category from "@/components//category/category";
import Search from "./search";
import Sign from "./sign";
import Cart from "./cart";
import Link from "next/link";
import { iProductVariantCart } from "@/components/product/interface";
import Sidebar from "./sidebar";
import { iMainCateGory } from "../category/interface";
import { useEffect, useState } from "react";
interface iHeaderNavi {
  category: iMainCateGory[];
  user: {
    fullName: string;
    account: string;
    phoneNumber: string;
    address: string;
  } | null;
  ls: iProductVariantCart[];
}
const HeaderNavi = ({ category: data, user, ls }: iHeaderNavi) => {
  const [show, setShow] = useState("");
  useEffect(() => {
    let before = window.scrollY;
    let after = 0;
    const f = (ev: Event) => {
      before = after;
      after = window.scrollY;

      if (after < before && window.scrollY > 250) {
        setShow("true");
      } else {
        setShow("");
      }
    };
    window.addEventListener("scroll", f);
    return () => {
      window.removeEventListener("scroll", f);
    };
  }, []);
  return (
    <header
      data-show={show}
      className=" w-full z-999 data-[show=true]:sticky top-0 left-0 "
    >
      <div className="bg-f pt-2.5 pb-2.5 lg:pt-4.5 lg:pb-3.5">
        <div className="max-w-350  mx-auto gap-5 flex max-lg:items-center max-lg:px-1.75">
          <div className="max-lg:flex-1">
            <div className="max-lg:flex items-center">
              <Sidebar />
              <div className="w-37.5 h-15 lg:h-17.5 px-3.75">
                <Link href="/" className="size-full">
                  <img
                    src="/logo.png"
                    className="h-full w-auto"
                    alt=""
                    srcSet=""
                  />
                </Link>
              </div>
            </div>
          </div>
          <div className=" lg:flex-1 px-3.75  fill-white">
            <div className="flex  items-center text-white  font-normal">
              <div className="hidden flex-1 lg:block">
                <Search />
              </div>
              <Sign {...user} />
              <Cart ls={ls} />
            </div>
          </div>
        </div>
        <div className="lg:hidden">
          <Search />
        </div>
      </div>
      <nav className="max-lg:hidden bg-a max-w-350 mx-auto  uppercase px-3.75">
        <ul className="text-f fill-f font-medium flex flex-wrap gap-0.75 justify-center">
          {data.map((v, i) => {
            return (
              <Category
                key={v.slug + i}
                id={v.id}
                slug={v.slug}
                categoryChidlren={v.categoryChidlren}
                nameCategory={v.nameCategory}
              />
            );
          })}
        </ul>
      </nav>
    </header>
  );
};

export default HeaderNavi;

...
export default function Category(p: MainCateGory) {
    ...
    const subCata = p.categoryChidlren.map((v) => {
        return (
            <li key={v.slug} className="px-4.5  py-2.25 text-[14px]">
                <Link className="text-text-shop font-normal" href={`/collections/${v.slug}`}>
                    {v.nameCategory}
                </Link>
            </li>
        )
    ...
    return (
        <li
            ...
            <Link href={`/collections/${p.slug}`} className="  py-3.75  inline-block">{p.nameCategory}
            ...
            <ul className={`${s ? "" : "hidden"} min-w-55 z-30  animate-cate text-[11px] absolute bg-white h-max top-full shadow-pro`}>
                {
                    subCata
                }
            </ul>

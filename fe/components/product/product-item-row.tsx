...
function ProductItem(p: iProduct) {
  ...
  return (
    ...
    >
      ...
      <div className="py-2.5 flex flex-col h-full items-center text-center font-medium px-3.5">
        ...
        <p className="text-[14px] leading-5 font-bold pb-2.5">
          {PriceFormat(p.mainPrice + "")}₫
        </p>
        <div className="mt-auto">
          <button
            className="uppercase cursor-pointer flex font-bold items-center justify-center 
                        space-x-3.5 rounded-full pl-3.75 border-1 border-white hover:border-f text-[12px] "
          >
            <span className="h-4">thêm vào giỏ</span>
...
export default function ProductItemRow(p: iProduct) {
  return (
    <Carousel
      opts={{
        align: "start",
      ...
      tabIndex={0}
      className="basis-1/2  lg:basis-1/5 grow-0 shrink-0 px-0.5 lg:px-1.75 "
    >
      <ProductItem {...p} />
    </Carousel>
  );

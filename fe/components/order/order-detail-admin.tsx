...
export default function OrderDetailAdmin(p: {
  ...
  const [statusMess, formStatusAction, pending1] = useActionState(
    updateStatusOrder,
    null,
  );
  const [statusPay, formPayAction, pending2] = useActionState(
    updatePayOrder,
    null,
  );
  useEffect(() => {
  ...
  return (
    <>
      <div className="pl-3.75">
        <h1 className="mb-3.75 text-2xl">Chi tiết đơn hàng</h1>
        <div className="space-y-3.5 mb-3.75">
          <p>
            <span className="font-bold pr-3">Mã đơn hàng:</span> {order.orderId}
          </p>
          <p>
            <span className="font-bold pr-3">Ngày đặt:</span>{" "}
            {dateFormat(order.orderTime)}
          </p>
          <LeafletButtom
            data={{ lat: parseFloat(order.lat), lng: parseFloat(order.lng) }}
            text={order.address}
          />
          ...
          <div className="flex items-center ">
            ...
            >
              ...
              <SelectContent>
                {orderStatus.map((v, i) => {
                  return <SelectItem value={v}>{v}</SelectItem>;
                })}
              </SelectContent>
            </Select>
          </div>
        ...
        <div className=" p-4 w-full overflow-x-auto">
          <table className="table-auto w-max lg:w-full ">
            ...
            <tbody>
              {items.map((p) => {
                return (
                  <tr>
                    ...
                    <td className="text-center pb-3.75">{p.quality}</td>
              ...
              <tr>
                <td colSpan={2} className="pt-3.75"></td>
                <td colSpan={3} className="pt-3.75">
                  <div className="flex space-x-3 justify-end">
                    <p className="font-bold text-2xl">Giá sản phẩm:</p>
      ...
      <AlertDialog open={openStatus.status}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              Bạn có chắc thực hiện việc cập nhật trạng thái hóa đơn
            </AlertDialogTitle>
            <AlertDialogDescription>
              Không thể hoàn tác hành động này. Thao tác này sẽ xóa vĩnh viễn
              tài khoản của bạn và xóa dữ liệu của bạn khỏi máy chủ của chúng
              tôi.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="flex space-x-2">
            ...
            <AlertDialogAction asChild>
              ...
              >
                ...
                >
                  <Button className="bg-f">Tiếp tục</Button>
                </SubmitButton>
              </Form>
            </AlertDialogAction>
          </AlertDialogFooter>
      ...
      <AlertDialog open={openPay.status}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              Bạn có chắc thực hiện việc cập nhật trạng thái thanh toán hóa đơn
            </AlertDialogTitle>
            <AlertDialogDescription>
              Không thể hoàn tác hành động này. Thao tác này sẽ xóa vĩnh viễn
              tài khoản của bạn và xóa dữ liệu của bạn khỏi máy chủ của chúng
          ...
          <AlertDialogFooter className="flex space-x-2">
            ...
            <AlertDialogAction asChild>
              <Form action={formPayAction} className="bg-white hover:bg-white">
                ...
                >
                  <Button className="hover:bg-red-500 bg-f">Tiếp tục</Button>
                </SubmitButton>
              </Form>
            </AlertDialogAction>
          </AlertDialogFooter>

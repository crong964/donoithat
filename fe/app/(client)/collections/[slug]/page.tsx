...
export default async function Colection(params: {
  ...
  const [data, brand] = await Promise.all([
    getProduct({ slug: param.slug, page: searchParams.page }),
    await getAllBrand(),
  ]);
  ...
  const ls: iNavi[] = [
    { name: "Danh mục", url: "/collections/all" },
    {
      name:
        param.slug == null || param.slug == "all"
          ? "Tất cả sản phẩm"
          : data.nameCate,
      url: `/collections/${param.slug}`,
    },
  ];
      ...
      <div className="flex">
        <aside className="max-lg:hidden basis-1/4 px-3.75">
          <Accordion type="multiple" className="w-full">
            <AccordionItem
              value="item-1"
              className="bg-white shadow-a rounded-sm mb-3.5"
            >
              ...
              <AccordionContent className="p-2.5  border-t border-boder">
                <ul className="text-[14px] font-medium tracking-letter leading-7">
                  <li>
                    <Link href="#">Được mua nhiều gần đây</Link>
                  </li>
                  <li>
            ...
            >
              ...
              <AccordionContent className="p-2.5  border-t border-boder">
                <ul className="text-[14px] font-normal tracking-letter leading-7">
                  {brand.map(({ brandId, brandName }) => {
                    return (
                      <li
                        id={brandId}
                        className="hover:border-f w-full cursor-pointer"
                      >
                        <label htmlFor={brandId}>
                          ...
                          <span className="w-full max-w"> {brandName}</span>
                        </label>
                      </li>
                    );
                  })}
        ...
        <div className="flex-1 lg:px-3.75">
          ...
          <div className="max-lg:px-3.75 py-2.5 lg:py-3.75">
            <div className="flex max-lg:flex-col lg:items-center">
              <h1 className="text-f font-bold text-[24px] leading-7.25">
                {data.nameCate || "Tất cả sản phẩm"}
              </h1>

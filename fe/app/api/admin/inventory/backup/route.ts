...
export async function POST(request: NextRequest) {
  ...
  try {
    api.post("/admin/inventory/backup", s);
  } catch (error) {}
...
export async function GET(request: NextRequest) {
  ...
  headers.append("Content-Disposition", 'attachment; filename="inventory.csv"'); // Set desired filename
  headers.append("Content-Type", "application/csv"); // Set appropriate Content-Type
  try {
    const da = await api.get("/admin/inventory/backup");
    const data = da.data as iBackupInventory[];
    data.forEach((v) => {
      for (const key in v) {
